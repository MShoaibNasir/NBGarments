    ...
    <div class="container py-4">
        <div class="card shadow-lg border-0">
            <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
                <h4 class="mb-0"><i class="bi bi-shield-lock me-2"></i> Bank Management</h4>
                @can('bank-create')
            ...
            <div class="card-body bg-light">
                <div class="table-container">
                    <table id="roleTable" class="table table-striped table-bordered align-middle">
                        ...
                        <tbody>
                            @forelse ($bank as $index => $data)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>{{ ucfirst($data->name) }}</td>
                                <td class="text-center action-btns">
                                    @can('bank-edit')
                                    <a href="{{ route('bank.edit', $data->id) }}" class="btn btn-success btn-sm">
                                        <i class="bi bi-pencil-square"></i> Edit
                                    </a>
                                    @endcan

                                    @can('bank-delete')
                                    <form action="{{ route('bank.delete', $data->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
